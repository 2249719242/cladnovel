...
- `high`/`critical` 的其余类别问题（连贯性/设定/角色/时间线/逻辑/节奏等，`ai_flavor` 除外）会由 review-pipeline 沉淀为「审查教训」写入 `.story-system/review_lessons.json`；同类问题反复出现会累加 `occurrences`，并在写前注入下一章写作合同的避雷项，实现自更新闭环。可用 `webnovel review-lessons stats/list/resolve` 查看与维护。
