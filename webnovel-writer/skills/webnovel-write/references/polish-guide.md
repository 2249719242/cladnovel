...
> **词频统计已下沉为确定性扫描，不要再靠人眼数词。**
> 优先运行 `python "${SCRIPTS_DIR}/webnovel.py" anti-ai-scan --file "<本章正文路径>"`（或 `--chapter N`），
> 它基于唯一真源词库 `scripts/data_modules/data/anti_ai_lexicon.json`，输出超阈值类别、命中样例、
> 结构层信号（句长方差/短句占比/重复短语/said-tag 占比）与修复方向。下方词表为人读参考，
> 词库以 JSON 为准；新增/调整高频词只改 JSON，勿在各 md 里另抄一份。
