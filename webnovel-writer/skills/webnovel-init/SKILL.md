...
设定深化（AI 强项，收集完成后主动执行）：
- 对用户给出的世界观做**推论链补全**：每条核心设定推演二阶后果（"设定 A 成立 → 必然出现 B"），把有剧情价值的推论作为设定提案给用户挑选
- **自洽性检查**：力量体系是否有代价与上限、经济是否有锚定、势力格局是否有制衡——发现漏洞列出来，连同修补方案
- **知识密集题材**（科幻/历史/医学/军事等）：主动用领域知识充实细节（技术原理、时代风貌、专业流程），并审校合理性；不确定的标注"需查证"，不编造
- 提案归提案，拍板归用户：所有深化产物经用户确认后才进入设定集
