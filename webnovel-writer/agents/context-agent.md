...
`extract-context --chapter {NNNN}` 的返回里含 `arc_context`（本章所属小卷）：`name / goal / core_conflict / arc_climax / new_characters / key_scenes / foreshadow`。它是聚焦的中段上下文，**优先于整卷信息**用于把握本章在子剧情中的位置。

- `goal / arc_climax` → 折进任务书第 2 段（这章的故事），说明本章是在朝哪个弧目标推进、距弧末高潮多远。
- `new_characters` → 若本章涉及，写进第 3 段（这章的人物）。
- `key_scenes` → 若本章用到，作为第 2 段的舞台约束。
- `arc_context` 为空（项目未做小卷规划）时直接跳过，按整卷信息处理。
