...
def main() -> None:
    ...
    p_review_lessons = sub.add_parser("review-lessons", help="转发到 review_lessons（审查教训库）")
    p_review_lessons.add_argument("args", nargs=argparse.REMAINDER)

    p_anti_ai_scan = sub.add_parser("anti-ai-scan", help="转发到 anti_ai_scanner（确定性反 AI 扫描）")
    p_anti_ai_scan.add_argument("args", nargs=argparse.REMAINDER)

    p_arc = sub.add_parser("arc", help="转发到 arc_planner（小卷/故事弧规划）")
    p_arc.add_argument("args", nargs=argparse.REMAINDER)
    ...
    # anti-ai-scan 支持 --file/--text 离线扫描，不强制依赖活动项目；
    # 仅当能解析到项目根时才前置 --project-root（供 --chapter 模式使用）。
    if tool == "anti-ai-scan":
        forward = []
        try:
            forward = ["--project-root", str(_resolve_root(args.project_root))]
        except FileNotFoundError:
            pass
        raise SystemExit(_run_data_module("anti_ai_scanner", [*forward, *rest]))
    ...
    if tool == "arc":
        raise SystemExit(_run_data_module("arc_planner", [*forward_args, *rest]))
    if tool == "review-lessons":
        raise SystemExit(_run_data_module("review_lessons", [*forward_args, *rest]))
    if tool == "migrate":
