...
class RuntimeContractBuilder:
    ...
    def build_for_chapter(self, chapter: int) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        ...
        anti_patterns = [*self._load_anti_patterns(), *self._load_review_lesson_rows()]
    ...
    def _load_review_lesson_rows(self) -> list[Dict[str, Any]]:
        """把审查教训库（高严重度问题）作为额外避雷项并入写作合同。"""
        from .review_lessons import lessons_for_injection

        return lessons_for_injection(self.project_root)
