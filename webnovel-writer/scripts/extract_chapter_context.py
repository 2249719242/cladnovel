...
def _load_arc_context(project_root: Path, chapter_num: int) -> Dict[str, Any]:
    """当前小卷（故事弧）上下文；无小卷规划时返回 {}（向后兼容）。"""
    _ensure_scripts_path()
    try:
        from data_modules.arc_planner import arc_context_for_chapter

        return arc_context_for_chapter(project_root, chapter_num)
    except Exception:
        return {}
...
def build_chapter_context_payload(project_root: Path, chapter_num: int) -> Dict[str, Any]:
    ...
    arc_context = _load_arc_context(project_root, chapter_num)
